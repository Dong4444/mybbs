...
<!--网页顶部end-->
  ...
  <div id="main">
    ...
    <div id="header">
      ...
      <?php else: ?>
        <div style="position:absolute;right:50px;bottom:0px;font-size:15px;">当前用户: <?=$_SESSION['userInfo']['uname']?></div>

        <div style="position:absolute;right:0px;bottom:0px;font-size:15px;"><a href="/index.php?m=home&c=login&a=logout">退出</a></div>
      <?php endif; ?>
      </div>
      <!--登陆部分start-->
...
<!--内容部分start-->
        <div class="content">           
            <form action="/index.php?m=home&c=post&a=save" method="post">
                <table height="60">
                 <tr>
                        <td><label>所属版块:</label></td>
                        <td>
                            <select name="cid">
                            <?php foreach($cates as $k=>$v):?>
                                <option <?php if($k==$cid){ echo 'selected';} ?> value="<?=$k?>"><?=$v?>
                                </option>
                            <?php endforeach;?>
                            </select>
                        </td>
